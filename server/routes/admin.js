...
// GET /api/admin/overview — ringkasan angka untuk dashboard admin
router.get('/overview', (req, res) => {
  res.json({ data: ringkasanAdmin() })
})

// ---------- Jadwal kerja ----------
router.get('/jadwal', (_req, res) => {
  res.json({ data: getJadwal() })
})

// PUT /api/admin/jadwal — ubah jam masuk (batas Terlambat) & jam pulang
router.put('/jadwal', (req, res) => {
  ...
  setSetting('jamMasukBatas', jamMasukBatas)
  setSetting('jamPulang', jamPulang)
  res.json({ data: getJadwal() })
})
...
router.post('/employees', (req, res) => {
  ...
  try {
    res.status(201).json({ data: buatKaryawan(d) })
  } catch {
...
router.put('/employees/:id', (req, res) => {
  const hasil = ubahKaryawan(Number(req.params.id), req.body || {})
...
router.delete('/employees/:id', (req, res) => {
  ...
  hapusKaryawan(Number(req.params.id))
...
router.get('/attendance', (req, res) => {
  ...
  res.json({ data: listSemuaAbsensi({ employeeId, dari, sampai }) })
})

router.put('/attendance/:id', (req, res) => {
  const hasil = ubahAbsensi(Number(req.params.id), req.body || {})
...
router.delete('/attendance/:id', (req, res) => {
  hapusAbsensi(Number(req.params.id))
  res.json({ data: { ok: true } })
})

// ---------- Kelola Izin/Cuti ----------
router.get('/leaves', (req, res) => {
  res.json({ data: listSemuaIzin() })
})
...
router.put('/leaves/:id', (req, res) => {
  ...
  const hasil = setStatusIzin(Number(req.params.id), status)
  if (!hasil) return res.status(404).json({ error: 'Pengajuan tidak ditemukan.' })
  res.json({ data: hasil })
})

router.delete('/leaves/:id', (req, res) => {
  hapusIzin(Number(req.params.id))
  res.json({ data: { ok: true } })
})

// ---------- Kelola Lembur ----------
router.get('/overtime', (req, res) => {
  res.json({ data: listSemuaLembur() })
})

router.put('/overtime/:id', (req, res) => {
  const status = req.body?.status
  if (!['Disetujui', 'Ditolak', 'Menunggu'].includes(status)) {
    return res.status(400).json({ error: 'Status tidak valid.' })
  }
  const hasil = setStatusLembur(Number(req.params.id), status)
  if (!hasil) return res.status(404).json({ error: 'Pengajuan lembur tidak ditemukan.' })
  res.json({ data: hasil })
})

router.delete('/overtime/:id', (req, res) => {
  hapusLembur(Number(req.params.id))
  res.json({ data: { ok: true } })
})
...
router.get('/notifications', (req, res) => {
  res.json({ data: listSemuaNotifikasi() })
})
...
router.post('/notifications', (req, res) => {
  ...
  if (!employeeId) {
    const hasil = kirimPengumuman({ judul: String(judul).trim(), pesan, jenis })
  ...
  const row = kirimNotifikasi({ employeeId: Number(employeeId), judul: String(judul).trim(), pesan, jenis })
  res.status(201).json({ data: notifToClient(row) })
})

// PUT — ubah isi pengumuman untuk semua penerima (status baca direset)
router.put('/notifications/grup/:grupId', (req, res) => {
  ...
  const hasil = ubahPengumuman(req.params.grupId, { judul, pesan, jenis })
...
router.delete('/notifications/grup/:grupId', (req, res) => {
  const jumlah = hapusPengumuman(req.params.grupId)
...
router.delete('/notifications/:id', (req, res) => {
  hapusNotifikasi(Number(req.params.id))
  res.json({ data: { ok: true } })
})
