      ...
      <div className="mb-2 grid grid-cols-4 gap-1.5 text-center">
        {STATUS_REKAP.map((s) => (
          <div key={s} className="min-w-0 rounded-2xl bg-slate-50 py-2 dark:bg-slate-800/60">
            <p className={`text-base font-extrabold leading-none ${WARNA_ANGKA[s]}`}>{pekan.rekap[s]}</p>
            <p className="mt-1 truncate text-[9px] font-semibold uppercase tracking-wide text-slate-400">{s}</p>
          </div>
        ))}
      </div>
      <p className="mb-3 text-center text-[10px] font-semibold text-slate-400">
        {pekan.rekap.masuk} dari {pekan.rekap.hariKerja} hari kerja · {pekan.rekap.persen}% kehadiran
      </p>
          ...
          >
            ...
            <span
              className={`text-[10px] font-bold leading-none ${
                d.hariIni ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-300 dark:text-slate-500'
              }`}
            >
              {d.tgl}
            </span>
