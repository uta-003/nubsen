...
export function adminUpdateJadwal(jamMasukBatas, jamPulang) {
  return request('/api/admin/jadwal', { method: 'PUT', body: { jamMasukBatas, jamPulang } })
}
...
export async function checkIn({ jam, lokasi, selfie }) {
  const data = await request('/api/attendance/check-in', {
    method: 'POST',
    body: { jam, lat: lokasi?.lat, lon: lokasi?.lon, alamat: lokasi?.alamat, selfie },
  })
  return mapRecord(data)
}

export async function checkOut({ jam, lokasi, selfie }) {
